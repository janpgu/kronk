...
kronk is a job scheduler that runs on a single SQLite file. No Redis, no message broker, no always-on daemon. You add one line to crontab, and kronk takes it from there: check what's due, run it, record the result, exit.
